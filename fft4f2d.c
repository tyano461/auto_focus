...
static inline bool is_pow2(int a)
{
    return (a & (a - 1)) == 0;
}

void NOINL realfft(int n, double **a)
{
    ...
    ERRRET(!is_pow2(n), "%d is not power of 2.", n);

    ip = malloc(sizeof(int) * (n + 2));
    w = malloc(sizeof(double) * n);
    ERRRET(!ip || !w, "malloc failed.");
    ip[0] = 0;
